...
*/
Class compact Extends CFLayout
{
	/**
	...
	function get_field($counter, $field_type, $field_title, $required, $pagenum, $options = array(), $caption = '', $value = null)
	{
		...
		{
			case 'text':
				$val      = ($value != null) ? $value : $field_title;
				$blurtext = str_replace('\'', '\\\'', $field_title) . $req;
				$onblur   = 'onblur="if (this.value.length == 0) { this.value = \''.$blurtext.'\'; }"';
				$onfocus  = ($value != null) ? '' : 'onfocus="if (this.value == \''.$blurtext.'\') { this.value=\'\'; }"';
				$field    = '<input type="text" class="text" name="fields['.$counter.']" value="'.$blurtext.'" '.$onfocus.' '.$onblur.' />';
				break;
			case 'textarea':
				$val      = ($value != null) ? $value : $field_title;
				$blurtext = str_replace('\'', '\\\'', $field_title) . $req;
				$onblur   = 'onblur="if (this.value.length == 0) { this.value = \''.$blurtext.'\'; }"';
				$onfocus  = ($value != null) ? '' : 'onfocus="if (this.value == \''.$blurtext.'\') { this.value=\'\'; }"';
				$field    = '<textarea name="fields['.$counter.']" '.$onfocus.' '.$onblur.'>'.$blurtext.'</textarea>';
				break;
			case 'checkbox':
				$field = '<div class="field_title">'.$field_title . $req.'</div>';
			...
			case 'terms':
				...
				$field = '<div class="field_title">' . $field_title . $req . '</div><textarea class="terms" readonly="readonly">'.implode("\n", $options).'</textarea><br />
			...
			case 'info':
				//$field = implode("\n", $options);
				$field = '<div class="title">'.$field_title.'</div>' . $_caption;
				break;
			case 'scale':
				...
				$field = <<<HTML
<div class="field_title">$field_title $req</div>
