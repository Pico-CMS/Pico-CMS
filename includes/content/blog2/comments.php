...
if ($action == 'post_new')
{
	...
	elseif (!$captcha->Verify(strtoupper($_POST['captcha'])))
	{
	...
	else
	{
		...
		if ($settings['allow_comments'] == 1)
		{
			$new_post = $db->insert('INSERT INTO `'.$blog_comments.'` (`post_id`, `name`, `active`, `date`, `ip_address`, `message`, `email`, `url`, `parent`) VALUES (?,?,?,?,?,?,?,?,?)',
				$post_id, $name, 1, time(), getenv('REMOTE_ADDR'), $message, $email, $url, $comment_id
			);
			...
			$link = 'http://'.$_SERVER['SERVER_NAME'] . $body->url('includes/content/blog2/comments.php?page_action=deny&deny='.$new_post);
		...
		{
			$new_post = $db->insert('INSERT INTO `'.$blog_comments.'` (`post_id`, `name`, `active`, `date`, `ip_address`, `message`, `email`, `url`, `parent`) VALUES (?,?,?,?,?,?,?,?,?)',
				$post_id, $name, 0, time(), getenv('REMOTE_ADDR'), $message, $email, $url, $comment_id
			);
			
			$_message = nl2br($message);
			$link = 'http://'.$_SERVER['SERVER_NAME'] . $body->url('includes/content/blog2/comments.php?page_action=approve&approve='.$new_post);
		...
		$mail->AddAddress($settings['moderator_address']);
		$mail->IsHTML(true);
...
HTML;
	...
	$op = Blog2_GetComments($post_id, $comment_layout) . Blog2_CommentForm($post_id);
	echo $op;
}
