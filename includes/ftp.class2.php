...
class Ftp
{
	 ...
	 */
	public function __construct($url = NULL, $ssl = false)
	{
		...
		if ($url) {
			...
			if ($ssl)
			{
				$this->sslconnect($parts['host'], empty($parts['port']) ? NULL : (int) $parts['port']);
			}
			else
			{
	...
	/**
	 * This classes chmod is stupid and broken, so this fixes it
	 * @param  string
	 * @param  octal
	 * @return void
	 */
	...
	{
		@ftp_chmod($this->resource, $mode, $file);
	}
}
